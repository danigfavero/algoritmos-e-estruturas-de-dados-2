...
public class MeuTST<Value extends Comparable<Value>> {
    ...
    public Iterable<String> keysWithPrefixByValue(String prefix) {
        ...
        RedBlackBST<Value, String> bst = new RedBlackBST<Value, String>();
        Queue<String> queue = new Queue<String>();
        Node<Value> x = get(root, prefix, 0);
        if (x == null)
            return queue;
        if (x.val != null)
            bst.put(x.val, prefix.toString());
        collect(x.mid, new StringBuilder(prefix), bst);
        while (!bst.isEmpty()) {
            Value maxKey = bst.max();
            String max = bst.get(maxKey);
            bst.deleteMax();
            queue.enqueue(max);
        }
        return queue;
    ...
    // funçao auxiliar para keysWithPrefixByValue()
    private void collect(Node<Value> x, StringBuilder prefix, RedBlackBST<Value, String> bst) {
        if (x == null)
            return;
        collect(x.left, prefix, bst);
        if (x.val != null)
            bst.put(x.val, prefix.toString() + x.c);
        collect(x.mid, prefix.append(x.c), bst);
        prefix.deleteCharAt(prefix.length() - 1);
        collect(x.right, prefix, bst);
    }
    ...
    public void delete(String key) {
        ...
        MeuTST<Value> tst = new MeuTST<Value>();
        Value val;
        for (String str : keys()) {
    ...
    public static void main(String[] args) {
        ...
        tst.put("shore", 8);
        tst.put("shells", 7);
        for (String str : tst.keysWithPrefixByValue("s"))
            StdOut.println("teste com S : " + str);
        StdOut.print("\n");
        for (String str : tst.keysWithPrefixByValue("sh"))
            StdOut.println("teste com SH : " + str);
        StdOut.print("\n");
        for (String str : tst.keysWithPrefixByValue("by"))
            StdOut.println("teste com BY : " + str);
        StdOut.print("\n");
        for (String str : tst.keysWithPrefixByValue("j"))
            StdOut.println("teste com J : " + str);
        StdOut.println("\n");
        tst.delete("sally");
        StdOut.println("--------DELETE SALLY--------");
