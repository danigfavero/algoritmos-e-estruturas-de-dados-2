...
readLine(FILE *infile)
{
    ...
    line = emalloc((size+1)*sizeof(char)); /* GetBlock(size + 1);*/
    while ((ch = getc(infile)) != '\n' && ch != EOF) {
        if (n == size) {
            size *= 2;
            nline = emalloc((size+1)*sizeof(char)); /* GetBlock(size + 1);*/
    ...
    line[n] = '\0';
    nline = emalloc((n+1)*sizeof(char)); /* (string) GetBlock(n + 1); */
    strcpy(nline, line);
    free(line); /* FreeBlock(line); */
    return nline;
}
...
/* emalloc
 *
 * O parametro de emalloc e do tipo size_t.
 * Em muitos computadores, size_t e equivalente a unsigned int.
 * A funçao emalloc nao esta em nenhuma biblioteca e e desconhecida
 * fora destas notas de aula.
 * Ela e apenas uma abreviatura conveniente.
...
/* ecalloc
 *
 * Os parametros de ecalloc sao do tipo size_t.
 * Em muitos computadores, size_t e equivalente a unsigned int.
 * A funçao ecalloc nao esta em nenhuma biblioteca e e desconhecida
 * fora destas notas de aula.
 * Ela e apenas uma abreviatura conveniente.
 * Fonte:  http://www.ime.usp.br/~pf/algoritmos/aulas/aloca.html
 *
 ...
 * copyString
 *
 * Recebe uma string S e faz uma copia de S. O espaço para a
 * copia e alocado por emalloc. Esta funçao foi inspirada na
...
copyString(String s)
{
   ...
   newstr = emalloc(strlen(s) + 1); /* +1 para o '\0' */
   strcpy(newstr, s);
   return newstr;
}
...
getNextToken(String linha)
{
    ...
    *str = '\0';
    ...
    palavra = emalloc(len*sizeof(char));
