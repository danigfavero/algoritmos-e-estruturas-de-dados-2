...
/* tabela de simbolos */
#include "binarysearchst.h"  /* initST(), freeST(), get(), put(), delete(), ... */
...
void
testeOperacoesST(BinarySearchST st);

void
testeOperacoes(BinarySearchST st);
...
main(int argc, char *argv[])
{
    ...
    /* crie a tabela de simbolos */
    st = crieST(arqTexto);

    /* teste operacoes */
    testeOperacoes(st);

    /* libere a memoria ocupada pela ST */
    freeST(st);
 ...
 *  si­mbolos em que as chaves sÃ£o as palavras do arquivo.
 *  O valor correspondente a cada palavra e o seu numero
 *  de ocorrencias.
 *
 *  As chaves da tabela de si­mbolo sao do tipo String
 *  e os respectivos valores do tipo Integer.
 *
 *  String e Integer estao definidos em util.h.
 */
BinarySearchST
crieST(FILE *arqTexto)
{
    ...
    while ((linha = getLine(arqTexto))) {
        ...
        palavra = getNextToken(linha); /* 1a. palavra */
        while (palavra) {
            Integer frequencia;
            /* pegue o numero de ocorrencia da palavra ate o momento */
            frequencia = get(st, palavra);
 ...
 *  TESTEOPEARCOES(ST)
 *
 *  RECEBE uma tabela de simbolos ST e testa varias operaçoes
 *  interativamente.
 *
 *  A funçao supoe que as chaves sao do tipo String e os
 *  valores sao do tipo Integer (ver util.h).
 */
void
testeOperacoes(BinarySearchST st)
{
    ...
    while ((linha = getLine(stdin))) {
        ...
        /*---------------------------------*/
        else if (!strcmp(operacao, MOSTRE_ST)) {
            visitST(st, visit);
        }
        ...
        /*---------------------------------*/
            ...
            /* operaçao necessita de argumento key */
...
/*---------------------------------------------------------------*/
/*
 *  I M P L E M E N T A Ç A O   D A S   F U N Ç O E S   DA
 *                     A U X I L I A R E S
 */
static void
mostreUso (char *nomePrograma)
